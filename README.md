...
### 1. Memory (메모리 제어)
* **커스텀 메모리 추적기:** `new`/`delete` 연산자를 오버로딩하여 메모리 할당/해제 로그를 추적하고 메모리 누수(Leak)를 방지.
* **객체 풀링 (Object Pool):** 테트리스 블록(Tetromino) 생성 시 매번 동적 할당을 하지 않고, 미리 할당된 풀(Pool)에서 재사용하여 오버헤드 최소화.
